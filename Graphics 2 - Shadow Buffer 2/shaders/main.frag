...
float pointIsInShadow( vec3 p, vec3 lightPos,
                      ...
                      int lightIndex)
{
    ...
    float numLit = 9.0;
    vec2 delta = 1.0/textureSize(sampler2DArray(shadowBuffer,nearestSampler), 0).xy;
    for(float dt=-1.0;dt<=1.0;dt+=1.0){
    for(float ds=-1.0;ds<=1.0;ds+=1.0){
    
       vec2 p_ = pp.xy + vec2(delta.s*ds, delta.t*dt);
       float shadowBufferDistance = texture( sampler2DArray(shadowBuffer,nearestSampler), vec3( p_.xy,float(lightIndex) )).r;
        shadowBufferDistance = mix(hitherYon[0],hitherYon[1], shadowBufferDistance);
...
void main(){
    ...
    float sin_theta_NV = asin((length(cross(N,V)))/(length(N) * length(V)));
    vec3 F0 = mix( vec3(0.04), c.rgb, MF);
    ...
    float shadowPercent = pointIsInShadow(
        worldPos,
        light_eyePos[i].xyz,
        light_viewProjMatrix[i],
        light_hitherYon[i].xyz,
        i
    );
    ...
    totaldp += dp * shadowPercent;
    totalsp += sp * shadowPercent;
    }

...
{
    GLTFMaterial mtl;
    Json material = J["materials"][materialIndex];
    mtl.name = material.get("name","");

    Json metallicRoughness = material["pbrMetallicRoughness"];
    if(metallicRoughness.hasKey("baseColorFactor")){
        Json tmp = metallicRoughness["baseColorFactor"];
    ...
    Json tmp = material.get("emissiveFactor",Json());
    if( tmp.is_null() )
    ...
    if( material.hasKey("normalTexture")){
        mtl.normalTexture = loadTexture(J,buffer,material["normalTexture"]);
    } else {
        mtl.normalTexture = loadDefaultTexture((const char*)periwinkle_png,sizeof(periwinkle_png),"periwinkle1x1");
    }

    mtl.pbrMetallicRoughness.metallicFactor = metallicRoughness.get("metallicFactor",1.0f);
    mtl.pbrMetallicRoughness.roughnessFactor = metallicRoughness.get("roughnessFactor",1.0f);

    if( metallicRoughness.hasKey("metallicRoughnessTexture"))
        mtl.pbrMetallicRoughness.metallicRoughnessTexture = loadTexture(J,buffer,metallicRoughness["metallicRoughnessTexture"]);
    else
        mtl.pbrMetallicRoughness.metallicRoughnessTexture = loadDefaultTexture((const char*)white_png,sizeof(white_png),"white1x1");

    if( material.hasKey("occlusionTexture"))
        mtl.occlusionTexture = loadTexture(J,buffer,material["occlusionTexture"]);
    else
        mtl.occlusionTexture = loadDefaultTexture((const char*)white_png,sizeof(white_png),"white1x1");

    extractExtras(material,&mtl);
    return mtl;
}

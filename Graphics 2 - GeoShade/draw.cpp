...
void draw(Globals& globs)
{
    ...
    globs.uniforms->set("shaftWidth", 0.005f);
    globs.uniforms->set("shaftDepth", 0.005f);
    globs.uniforms->set("shaftHeight", 0.01f);
    globs.uniforms->set("time", globs.time);
    //bind descriptor set
    ...
    for (auto& m : globs.allMeshes) {
        if (m->name != "torus" && m->name != "ground")
            m->draw(cmd, globs.descriptorSet, globs.pushConstants);
    }

    globs.grassPipeline->use(cmd);
    for (auto& m : globs.allMeshes) {
        if (m->name == "torus" || m->name == "ground")
            m->draw(cmd, globs.descriptorSet, globs.pushConstants);
    }

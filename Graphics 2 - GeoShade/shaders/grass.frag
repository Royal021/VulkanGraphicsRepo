...
vec3 doBumpMapping(vec3 b, vec3 N)
{
    ...
    vec3 L = lightPositionAndDirectionalFlag[0].xyz - worldPos * lightPositionAndDirectionalFlag[0].w;
    if (dot(N,L)<0){
         N=-N;
    }
    vec3 L = lightPosition - worldPos*positional;
...
void main(){
    ...
    vec4 c = texture( sampler2DArray(baseColorTexture,texSampler),
                      vec3(texcoord.xy,animationFrame) );
    c = c * baseColorFactor;
    ...
    float mixfactor = texcoord[2];
    color.rgb *= mix( 0.4,1.0,mixfactor);
